...
server <- function(input, output)
{
  ...
  xAxisModal <- function(failed=FALSE)
  {
    modalDialog(
      ...
      footer=shiny::tagList(
        ...
        shiny::actionButton("xAxisButtonOk", "OK")
      )
    )
  }
  ...
  output$plot <- shiny::renderPlot({
    ...
    if (is.null(state$image)) {
      ...
      if (input$grid == "fine") {
        abline(h=seq(-3, 3, 0.05),
               col='magenta',
               lty="dotted")
        abline(v=seq(-3, 3, 0.05 * asp),
               col='magenta',
               lty="dotted")
      } else if (input$grid == "medium") {
        abline(h=seq(-3, 3, 0.1),
               col='magenta',
               lty="dotted")
        abline(v=seq(-3, 3, 0.1 * asp),
               col='magenta',
               lty="dotted")
      } else if (input$grid == "coarse") {
        abline(h=seq(-3, 3, 0.2),
               col='magenta',
               lty="dotted")
        abline(v=seq(-3, 3, 0.2 * asp),
               col='magenta',
               lty="dotted")
      }
      ## if (input$guides == "On") {
